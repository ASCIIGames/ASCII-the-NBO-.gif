...
public class LevelGeneration : MonoBehaviour
{
	...
	void Start()
	{
		...
		{ // make sure we dont try to make more rooms than can fit in our grid
			numberOfRooms = int(Math.round((worldSize.x * 2) * (worldSize.y * 2));
		}
		gridSizeX = int(Math.round(worldSize.x)); //note: these are half-extents
		gridSizeY = int(Math.round(worldSize.y));
	...
	Vector2 NewPosition()
	{
		...
		do
		{
			int index = int(Math.round(Random.value * (takenPositions.Count - 1)); // pick a random room
	...
	{ // method differs from the above in the two commented ways
		...
		do
		{
			...
			do
			{
				...
				index = int(round(Random.value * (takenPositions.Count - 1)));
				inc++;
